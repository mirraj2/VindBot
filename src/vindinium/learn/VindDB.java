...
public class VindDB {
  ...
  public List<Scenario> getScenarios() {
    ...
    for (Row row : db.select("SELECT * FROM lessons")) {
      ...
      ret.add(new Scenario(state, goodMoves, badMoves));
    }
    return ret;
  }
  ...
  public void logBadMove(GameState state) {
    ...
    db.insert("lessons", new Row().with("json", state.json.toString()).with("good_moves", "").with("bad_moves", dir));

    logger.info("Logged bad move.");
  }

  public void logGoodMove(GameState state, Dir dir) {
    db.insert("lessons", new Row().with("json", state.json.toString()).with("good_moves", dir).with("bad_moves", ""));
  ...
  public static class Scenario {
    ...
    public Scenario(GameState state, EnumSet<Dir> goodMoves, EnumSet<Dir> badMoves) {
      this.state = state;
